...
test('it renders with default page size', function(assert) {
  ...
  assert.equal(this.$('.ember-power-select-selected-item').text().trim(), '5',
    'Selected value should be 5');
});

test('it renders with given page size', function(assert) {
  this.set('pageSize', 10);
  this.render(hbs`{{core/page-size pageSize=pageSize}}`);
  assert.equal(this.$('.ember-power-select-selected-item').text().trim(), '10',
...
test('Can select another page size (10)', function(assert) {
  ...
  this.set('externalAction', actual => {
    assert.equal(actual, this.get('expectedValue'),
      `submitted value is passed to external action (${actual})`);
  });
...
test('It renders with custom page sizes', function(assert) {
  ...
  assert.equal(this.$('.ember-power-select-selected-item').text().trim(), 6,
    'Selected value should be 6');

  this.set('expectedValue', 11);
  nativeMouseDown('.ember-power-select-trigger');
  nativeMouseUp('.ember-power-select-option:nth-child(2)');

  assert.equal(this.$('.ember-power-select-selected-item').text().trim(), 11,
    'Selected value should be 11');
});
