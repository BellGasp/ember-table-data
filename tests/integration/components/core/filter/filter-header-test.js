...
moduleForComponent('core/filter/filter-header', 'Integration | Component | core/filter/filter header', {
  integration: true
});
...
test('it render the available component (button)', function(assert) {
  ...
  assert.equal(this.$('.testAddButton').length, 1, 'It render the addButton when using the component');
});
